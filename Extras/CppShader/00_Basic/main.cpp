...
int main()
{
    ...
    GLFWwindow* window = glfwCreateWindow(800, 600, "00 Basic", nullptr, nullptr);
    if (window == nullptr)
    {
    ...
    try
    {
        ...
        const std::string vertexSource = MakeMacOSCompatibleSource(
            ReadTextFile(std::string(GLSL_DIRECTORY) + "/00_Basic.vert"));
        const std::string fragmentSource = MakeMacOSCompatibleSource(
            ReadTextFile(std::string(GLSL_DIRECTORY) + "/00_Basic.frag"));
    ...
    // 1頂点をposition.xyz + normal.xyzの順で並べたinterleaved data。
